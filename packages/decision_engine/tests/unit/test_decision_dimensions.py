...
def test_c_mixed_evidence_survives_as_conflict() -> None:
    ...
    assert dims.stability.caution_evidence_ids
...
def test_g_missing_evidence_is_insufficient_not_invented_confidence() -> None:
...
def test_golden_mixed_conflict_dimensions() -> None:
    ...
    conflicted = [
        key
        for key in DIMENSION_KEYS
        if getattr(snapshot.dimensions, key).conflicted
    ]
    assert conflicted
