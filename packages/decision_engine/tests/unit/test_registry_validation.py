...
def test_canonical_registry_contains_exactly_three_authorized_types() -> None:
...
def test_canonical_type_metadata_is_available() -> None:
    ...
    assert interview.allowed_modes == ("evaluate_date",)
...
def test_fourth_type_is_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(RegistryLoadError, match="validation failed|exactly 3"):
