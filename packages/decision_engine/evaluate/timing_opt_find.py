"""Timing Opt Family FIND runtime (shared spine).

Scores every inclusive day in a framed range under identical natal evidence
and type action profile, then builds contiguous Favorable+ windows.
Does NOT assemble N independent Evaluate packages or synthetic Compare options.
"""
    ...
    payload: dict[str, Any] = {
        **build_find_base_envelope(
        ...
        "explainability": {
            ...
            "why_not": "Without natal inputs, launch timing windows cannot be scanned.",
            "assumptions": [],
...
def find_timing_opt(
    ...
    *,
    ...
    """Shared Timing Opt Family FIND spine for an allowlisted type config."""
    ...
    for day in iter_inclusive_dates(range_start, range_end):
        ...
        if request.action_type != config.action_type:
            raise ValueError(
                ...
                "TimingOptTypeFindConfig.action_type"
            )
