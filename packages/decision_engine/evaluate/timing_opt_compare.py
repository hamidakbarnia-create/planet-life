"""Timing Opt Family COMPARE runtime (shared spine).
    ...
    *,
    ...
    payload: dict[str, Any] = {
        **build_compare_base_envelope(
        ...
        "tradeoffs": {
            "items": [
                "Higher timing score may reduce scheduling flexibility among candidates."
            ]
        },
        "risks": {
            "items": [
...
def compare_timing_opt(
    ...
    *,
    ...
    """Shared Timing Opt Family COMPARE spine for an allowlisted type config."""
    if config.family_id != "timing_opt":
        raise ValueError(
            "compare_timing_opt requires family_id='timing_opt'; "
    ...
    for option in options:
        ...
        if request.action_type != config.action_type:
            raise ValueError(
                ...
                "TimingOptTypeCompareConfig.action_type"
            )
