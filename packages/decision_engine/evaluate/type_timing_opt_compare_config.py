"""Explicit Timing Opt TypeCompareConfig allowlist for COMPARE runtimes.

Family runtime alone never activates a Decision Type. Only types registered
here may execute timing_opt COMPARE. This is intentionally NOT
"every type with family_id=timing_opt" and NOT every type that allows
...
class TimingOptCompareSemantics(Protocol):
    """Type-specific Package wording for Timing Opt COMPARE."""
...
class TimingOptTypeCompareConfig:
    """Allowlisted Decision Type configuration for Timing Opt COMPARE."""
    ...
    semantics: TimingOptCompareSemantics
    ...
    def __post_init__(self) -> None:
        if self.family_id != "timing_opt":
            raise ValueError(
                "TimingOptTypeCompareConfig requires "
...
_TIMING_OPT_TYPE_COMPARE_CONFIGS: dict[str, TimingOptTypeCompareConfig] = {}


def register_timing_opt_type_compare_config(
    config: TimingOptTypeCompareConfig,
    ...
    existing = _TIMING_OPT_TYPE_COMPARE_CONFIGS.get(config.decision_type_id)
    if existing is not None and existing is not config:
        raise ValueError(
            f"duplicate TimingOptTypeCompareConfig for {config.decision_type_id}"
        )
    _TIMING_OPT_TYPE_COMPARE_CONFIGS[config.decision_type_id] = config
    return config


def get_timing_opt_type_compare_config(
    decision_type_id: str,
) -> TimingOptTypeCompareConfig | None:
    return _TIMING_OPT_TYPE_COMPARE_CONFIGS.get(decision_type_id)


def clear_timing_opt_type_compare_configs_for_tests() -> None:
    _TIMING_OPT_TYPE_COMPARE_CONFIGS.clear()


def bind_timing_opt_compare_runtime(
    config: TimingOptTypeCompareConfig,
...
__all__ = [
    "TimingOptCompareSemantics",
    "TimingOptFamilyId",
    "TimingOptTypeCompareConfig",
    "bind_timing_opt_compare_runtime",
    "clear_timing_opt_type_compare_configs_for_tests",
