"""Versioned DecisionEvidence → DecisionDimensions mapping (dimensions.v1-shadow).
...
(kind, polarity, contribution, transit_body, natal_target, retrograde, angle)
onto named dimensions.
...
Routing (v1-shadow):
- Generic body→dimension rows apply to ``transit_body`` only.
- Kind overlays still apply.
- ``natal_target`` does **not** inherit ``BODY_DIMENSION_WEIGHTS``.
- Natal-target effects require an explicit row in
  ``NATAL_TARGET_DIMENSION_WEIGHTS`` (empty in this version).
"""
...
MAPPING_VERSION: Final[str] = "dimensions.v1-shadow"
SEMANTIC_STATUS: Final[str] = "experimental_shadow"
...
# Used only to normalize evidence_strength (mass), not as prediction confidence.
...
# Body → (dimension, weight). Applies to transit_body only.
...
# Kind overlays on top of transit-body mapping (existing engine semantics).
...
# Explicit natal-target overlays. Empty in v1-shadow: natal_target is identity
# only and MUST NOT copy BODY_DIMENSION_WEIGHTS. Future target-specific rules
# belong here after review — never via automatic generic-body inheritance.
NATAL_TARGET_DIMENSION_WEIGHTS: Final[dict[str, tuple[tuple[str, float], ...]]] = {}
...
for _kind, _rows in KIND_DIMENSION_WEIGHTS.items():
    for _dim, _weight in _rows:
        assert _dim in DIMENSION_KEYS
        assert 0.0 < _weight <= 1.0
for _body, _rows in NATAL_TARGET_DIMENSION_WEIGHTS.items():
    assert _rows != BODY_DIMENSION_WEIGHTS.get(_body)
    for _dim, _weight in _rows:
        assert _dim in DIMENSION_KEYS
        assert 0.0 < _weight <= 1.0
