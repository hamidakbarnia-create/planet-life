...
export function PathfinderSelectedLineCard({
  ...
  const body = meaning ?? copy.meaning;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <span
          className="fi shrink-0 rounded-full px-2 py-0.5 text-[9px] text-white/55"
      ...
      <details data-testid="pathfinder-geometry-demo-provenance" className="mt-2">
        <summary className="fi cursor-pointer text-[10px] text-white/45">{technicalProvenance}</summary>
        <p className="fi mt-1.5 text-[10px] leading-relaxed text-amber-200/80">
          {status ?? PATHFINDER_SUN_LINE_STATUS}
        </p>
