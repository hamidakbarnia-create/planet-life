"""Offer-negotiation intake transport over Decision Case SoR."""
...
# EVALUATE is the only mode this type supports.
_OFFER_NEGOTIATION_MODE = "evaluate_date"


def _require_offer_negotiation(case: CaseRecord) -> None:
    if case.decision_type_id != OFFER_NEGOTIATION_DECISION_TYPE_ID:
...
def save_offer_negotiation_answers(
    repo: DecisionCaseRepository,
    *,
    case_id: UUID,
    ...
    _require_offer_negotiation(case)
    ...
    merged = merge_offer_negotiation_intake(current, answers)
    ...
    evaluation = evaluate_offer_negotiation_intake(intake)
    if (
...
def complete_offer_negotiation_intake(
    repo: DecisionCaseRepository,
    *,
    case_id: UUID,
    owner_subject_id: str,
